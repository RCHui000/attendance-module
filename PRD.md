...
| UI | Tailwind CSS v4 + shadcn/ui 风格组件 + Lucide React；全局字体使用微软雅黑 |
| 图表 | Recharts |
...
| NAS 部署 | 局域网 Docker 容器栈，当前测试镜像 `attendance-module:V0.16.28` |
| 云端部署 | 阿里云 ECS，Nginx 反向代理，当前生产镜像 `approval-app:V0.16.28` |
...
| 页面 | 路由 | 说明 |
| --- | --- | --- |
| 登录页 | 未登录默认 | 登录、修改密码、密码明文切换；登录背景使用分层波浪动画，登录卡片带轻微透明效果 |
| 我的周表 | `/timesheet` | 员工填写项目工时；按自然月边界拆分周表区间，保持周一到周日固定列；OT 行预留但当前锁定 |
| 请假申请 | `/leave` | 受资源权限控制的请假功能入口；当前为占位页，暂不接后端接口 |
| 数据看板 | `/dashboard` | 指标卡、项目汇总、BI 项目/部门/人员视角 |
| 审批中心 | `/review` | 待审核、已审核、周表详情；PC 端行内展开详情，移动端竖向展开详情；admin 可维护合同审批模板并实时预览流程图；OT 审批能力保留但当前业务不启用 |
| 项目列表 | `/report` | PC 端左侧项目目录、右侧固定配置页；移动端默认只展示项目目录，点击项目后以弹窗卡片维护配置；维护项目编码、服务类型、项目名称、PM/CC/PMCC 负责人、财务与工时统计 |
| 员工与组织 | `/employees` | 系统管理与权限配置并列；员工资料、合同薪酬、组织结构、部门负责人、平台权限矩阵 |
| 应用中心 | `/apps` | 受资源权限控制的应用入口；卡片式应用目录，整卡可点击；admin 可新增、编辑、启停和删除应用；内网应用在图标角标显示“内网” |
...
应用中心用于统一进入公司内部工具和业务应用。当前已落地数据库配置、前端卡片、admin 维护能力和内网应用标记。

当前口径：

- 普通用户只看到已启用且有权限访问的应用卡片。
- 应用卡片仅展示用户需要识别的信息：图标、名称和必要标签；内网地址等实现细节不直接暴露在卡片文本中。
- 内网应用在图标右下角叠加白底黑字“内网”角标。
- 卡片整体作为点击目标，带按压/释放动画，减少“按钮到底点哪里”的不确定性。
- admin 在应用中心内维护应用名称、URL、图标、内网标记、排序和启用状态。
- 当前首个应用为“招标信息聚合”，由生产 Nginx 在 `/apps/tender/` 反向代理到独立的 `tender-app` 服务，历史采集数据通过持久化目录保留。
...
| 前端路径 | 实际实现 | 说明 |
| --- | --- | --- |
| 登录 | Supabase Auth SDK `signInWithPassword` | 支持登录名、姓名、员工编号、邮箱解析；`serve_spa.py` 代理兜底拦截停用/离职账号 |
...
| `GET /api/timesheet` | PostgREST 平铺查询 | 周表、明细、加班；按传入 `weekStart` 解析当前周/月边界拆分后的填报区间 |
| `POST /api/timesheet/save` | PostgREST 写入 | 草稿/退回状态保存 |
| `POST /api/timesheet/action` | `psa_timesheet_action` RPC | 周表提交、审批、退回、重开 |
| `GET /api/timesheet-detail` | PostgREST 平铺查询 | 审批详情页周表明细；PC 和移动端共用 |
...
| `GET /api/apps` | PostgREST 聚合 | 读取应用中心卡片，按排序、启用状态和权限过滤 |
| `POST /api/apps/save` | PostgREST upsert | admin 新增或编辑应用卡片、链接、内网标记、图标、排序和启用状态 |
| `POST /api/apps/delete` | PostgREST 软删除 | admin 删除应用中心条目 |
...
| `GET /api/numbering/employee` | 受控编号生成 | 返回下一个员工编号 |
| `GET /api/numbering/project` | 受控编号生成 | 返回指定服务类型的下一个项目编号 |
...
| `app_center_items` | 应用中心卡片配置，支持内网标记、排序、启停和软删除 |
| `timesheets` | 周表主表 |
...
- 每名员工在同一自然周/自然月拆分区间内只能有一张周表。
- 周表以自然周为基础，但跨月周需要按自然月边界拆分成多张表，例如月底只展示周一到周二时，周三到周日保持空列。
- 顶部星期行固定显示周一到周日，拆分后的日期仍落在真实星期位置，不能因为跨月截断而错位。
- 每天所有项目工日合计不得超过 1 工日。
- 出勤工日校验按当前周表实际包含的星期数量计算；包含星期日时默认减 1，不包含星期日则不减，再扣除法定节假日。
- 每周普通工日最多 7 工日；周日上班按普通工日处理，不走 OT。
- 项目行/项目块合计显示 2 位小数，避免个位数百分比在审批时被 0.1 工日展示精度掩盖。
- 周表默认保留一行空项目块；意图识别只在失焦、Tab 或 Enter 后触发，不在实时输入中触发。
- 当某日合计不足 100% 时可自动追加空项目块；当日合计达到 100% 后不再追加；自动追加的空行必须可删除。
- 移动端项目搜索框、备注和数字输入使用 16px 字号，避免浏览器自动放大导致抽屉或浮层错位。
- 移动端项目块审批状态压缩为彩色状态点，PC 端继续显示图标和文字。
...
- 移动端使用压缩导航和纵向信息流，不复用 PC 端左右分栏承载密集表单。
- 数据看板和 BI 面板在移动端优先展示周期筛选、指标摘要和可纵向滚动的对象列表，避免横向挤压图表。
- 审批中心移动端不再使用侧弹抽屉展示周表详情，而是在任务卡片下方竖向展开，与 PC 端行内详情保持同一信息层级。
- 项目页移动端默认只展示项目目录；项目配置进入弹窗卡片编辑。
- 应用中心使用大图标加底部文字的卡片网格，卡片整体可点击并提供点击动画反馈。
...
| 043 | `043_v015_graph_rls_policy.sql` | 补齐 Approval Graph RLS 策略 |
| 044 | `044_v015_approval_graph_history_repair.sql` | 修复备份恢复后的待审/已审历史数据展示 |
| 045 | `045_project_signed_date.sql` | 项目签约日期字段 |
...
| 059 | `059_role_sidebar_order.sql` | 平台角色侧边栏顺序配置 |
| 060 | `060_fix_system_management_sidebar_resource.sql` | 修复系统管理资源与侧边栏权限映射 |
| 061 | `061_sync_pm_project_roles_to_platform_roles.sql` | 同步 PM 项目角色到平台角色相关数据 |
| 062 | `062_fix_pm_role_sync_ambiguity_and_backfill.sql` | 修复 PM 角色同步歧义并回填历史数据 |
| 063 | `063_user_roles_login_read_grant.sql` | 登录态读取用户角色授权补齐 |
| 064 | `064_login_alias_resolution_rpc.sql` | 登录别名解析 RPC |
| 065 | `065_report_write_project_rls.sql` | 报表/项目写入 RLS 对齐 |
| 066 | `066_project_write_grants.sql` | 项目写入授权补齐 |
| 067 | `067_permission_effects_and_rls_alignment.sql` | 权限配置效果与 RLS 对齐 |
| 068 | `068_workflow_graph_grants.sql` | Approval Graph 相关授权补齐 |
| 069 | `069_multi_pm_department_owners.sql` | 多 PM 部门负责人支持 |
| 070 | `070_timesheet_project_revision_entry_rls.sql` | 周表项目块重提交编辑 RLS |
| 071 | `071_project_block_resubmit_integrity.sql` | 项目块重提交完整性约束 |
| 072 | `072_timesheet_action_pending_assignee_guard.sql` | 周表动作当前待办指派人校验 |
| 073 | `073_admin_reject_active_approval_node.sql` | admin 退回当前激活审批节点支持 |
| 074 | `074_rejected_project_change_rescope.sql` | 退回项目变更后重新限定项目块范围 |
| 075 | `075_engineering_office_org_tree.sql` | 工程部办公室组织树补齐 |
| 076 | `076_admin_explicit_timesheet_node_actions.sql` | admin 显式周表节点动作 |
| 077 | `077_project_revision_event_fk_cleanup.sql` | 项目修订事件外键清理 |
| 078 | `078_database_redundancy_phase1_cleanup.sql` | 数据库冗余一期清理 |
| 079 | `079_restore_postgrest_runtime_grants.sql` | 恢复 PostgREST 运行授权 |
| 080 | `080_allow_contract_approval_node_scope.sql` | 允许合同审批节点 scope |
| 081 | `081_publish_approval_graph_realtime.sql` | 发布 Approval Graph Realtime 变更 |
| 082 | `082_ensure_psa_realtime_tenant.sql` | 确保 PSA Realtime tenant |
| 083 | `083_ensure_approval_realtime_tenant.sql` | 确保审批 Realtime tenant |
| 084 | `084_ensure_public_realtime_tenant.sql` | 确保 public Realtime tenant |
| 085 | `085_app_center_items.sql` | 应用中心表、权限资源和首个内网应用配置 |
...
- Realtime 已在 NAS/云端部署，`081` 至 `084` 迁移补齐 Approval Graph 与 public tenant publication；前端保留 BroadcastChannel 和查询刷新作为 fallback。
- 员工提交周表、审批节点变更、项目块退回/重提交后，应实时 invalidate `approvals`、`timesheet`、`reports`、`dashboard` 等查询组，负责人待审批列表应自动刷新。
...
- NAS 测试环境已部署 `attendance-module:V0.16.28`，健康检查通过。
- 阿里云 ECS 生产环境已部署 `approval-app:V0.16.28`，健康检查通过。
- 域名 `xpjs.asia` 与 `www.xpjs.asia` 已配置 A 记录到云服务器公网 IP。
- `https://xpjs.asia/` 为当前生产访问入口。
...
- 手机浏览器可访问。
- 移动端周表输入不会因搜索框或备注框聚焦而自动放大。
- 移动端审批中心周表详情以竖向展开展示，不使用侧弹遮罩。
- 移动端项目页点击项目后以弹窗卡片编辑配置。
- Nginx 日志和容器日志可查看。
...
- admin 可在应用中心新增、编辑、启停和删除应用；非编辑权限用户不显示维护入口。
