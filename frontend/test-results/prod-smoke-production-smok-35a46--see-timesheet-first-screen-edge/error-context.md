...
- Name: prod-smoke.spec.ts >> production smoke acceptance >> employee can log in and see timesheet first screen
- Location: e2e\prod-smoke.spec.ts:118:3

# Error details

```
Error: Could not authenticate a normal employee with password 123456. Tried: 惠若超, 王长志, 陈京京, 赵嘉琪. Set E2E_EMPLOYEE_LOGIN/E2E_EMPLOYEE_PASSWORD or ask the main agent to create/reset a normal employee account.
```
