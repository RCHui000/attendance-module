-- Assert project owner assignments stay synchronized with platform roles.
...
DECLARE
  ...
  v_role text;
BEGIN
  SELECT pg_get_functiondef('public.psa_sync_project_platform_roles(bigint[])'::regprocedure)
  ...
  LOOP
    IF position(quote_literal(v_role) in v_sync_definition) = 0 THEN
      RAISE EXCEPTION 'psa_sync_project_platform_roles does not account for role %', v_role;
    END IF;
  END LOOP;

  IF position('project_department_owners' in v_sync_definition) = 0 THEN
    RAISE EXCEPTION 'psa_sync_project_platform_roles does not account for project_department_owners';
  END IF;

  IF position('admin' in v_sync_definition) = 0 OR position('director' in v_sync_definition) = 0 THEN
    RAISE EXCEPTION 'psa_sync_project_platform_roles must preserve admin/director roles';
  END IF;

  IF position('psa_sync_project_platform_roles' in v_save_definition) = 0 THEN
    RAISE EXCEPTION 'psa_save_project must call psa_sync_project_platform_roles after owner changes';
  END IF;
END;
$$;
