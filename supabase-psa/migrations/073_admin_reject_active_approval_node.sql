-- V0.15.3 hotfix: admins may reject an active approval node from the
-- admin review queue even when they are not the node's pending assignee.
-- Normal users are still required to be pending assignees.
...
BEGIN
  ...
  RETURN jsonb_build_object('ok', true, 'node_id', p_node_id, 'result', 'rejected');
END;
$$;

ALTER FUNCTION public.reject_node(bigint, bigint, text, text, text, text) OWNER TO postgres;
REVOKE ALL ON FUNCTION public.reject_node(bigint, bigint, text, text, text, text) FROM public, anon, authenticated;
