-- Ensure the cloud Realtime tenant matches the approval-realtime app name.
...
WHERE external_id = 'realtime-dev'
  AND NOT EXISTS (
    SELECT 1 FROM public.tenants WHERE external_id = 'approval_realtime'
  );
...
WHERE target.external_id = 'approval_realtime'
...
WHERE source.tenant_external_id = 'realtime-dev'
  AND NOT EXISTS (
    ...
    WHERE existing.tenant_external_id = 'approval_realtime'
...
SET
  ...
  updated_at = now()
FROM public.extensions source
WHERE target.tenant_external_id = 'approval_realtime'
