-- Ensure reverse-proxied /realtime/socket requests have a matching tenant.
...
WHERE external_id = 'realtime-dev'
  AND NOT EXISTS (
    SELECT 1 FROM public.tenants WHERE external_id = 'realtime'
  );
...
WHERE target.external_id = 'realtime'
...
WHERE source.tenant_external_id = 'realtime-dev'
  AND NOT EXISTS (
    ...
    WHERE existing.tenant_external_id = 'realtime'
...
SET
  ...
  updated_at = now()
FROM public.extensions source
WHERE target.tenant_external_id = 'realtime'
